...
class Graph:
    ...
    def wrapper(self, list):
        ...
        for i in range(len(list) - 1):
            count = i + 1
            res["count"] = {}
            res["count"]["cdntSx"] = buildings[int(list[i]) - 1]["bx"]
            res["count"]["cdntSy"] = buildings[int(list[i]) - 1]["by"]
            res["count"]["cdntTx"] = buildings[int(list[count]) - 1]["bx"]
            res["count"]["cdntTy"] = buildings[int(list[count]) - 1]["by"]
            res["count"]["speed"] = 5
            res["count"]["duration"] = abs(buildings[int(list[count]) - 1]["by"] - buildings[int(list[i]) - 1]["by"]) + abs(
                buildings[int(list[count]) - 1]["bx"] - buildings[int(list[i]) - 1]["bx"]) * 1.0 / res["count"]["speed"]
...
if __name__ == '__main__':
    ...
    graph1 = Graph()
    route = graph1.a_star_algorithm_time('9', '13')
